...
端口转发"0.0.0.0:1000"->"127.0.0.1:1080" 密码123
...
最终配置1条：
>端口转发"127.0.0.1:1080"->"110.1.1.1:110"密码123
...
最终配置：
>socks代理127.0.0.1:1080  
MPS资源：127.0.0.1:1080发送到110.1.1.1:110 mpsname:officesvr
...
最终配置2条：
>MPS服务0.0.0.0:110  
MPS用户127.0.0.1:1080 发送到127.0.0.1:110 mpsname:officesvr
...
2016.11.11
状态信息增加内存状态
代码优化：使用chan提高socks5代理的并发性能，降低并发纤程的使用。  
2016.11.14
再次尝试分离收发纤程，并使用chan复用收发缓存，减少内存重复分配和提高利用率。
为端口转发增加了多端口转发的功能，比如希望A端口收到的数据转发给多个代理服务器。用";"分割。  
2016.11.15
判断在多端口转发的情况下，对连接失败的重新处理。  
2016.11.18
改写状态信息的结构，增加反馈多端口转发的状态。  
2016.11.23
重新改写了tcp-udp-tcp的实现，貌似更稳定了。  
2016.12.5
降低资源错误的重试频率
代码优化。  
2017.1.6  
发送缓存有可能对数据的准确性造成影响，暂时关闭  
2017.1.24  
提高代理的并发性能  
